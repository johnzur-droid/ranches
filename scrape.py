...
function groupByStatus() {
  ...
  for (const [id, L] of Object.entries(state)) {
    ...
    // Townhouses: unreviewed go to townhouse tab; reviewed flow into normal tabs
    if (isTownhouse(L)) {
      if (!s || s === "new") {
...
function renderAll() {
  ...
  // Tab: Townhouses — unreviewed only (favorites/maybe/both flow to their normal tabs)
  const thPane = document.getElementById("tab-townhouse");
  if (groups.townhouse.length === 0) {
    thPane.innerHTML = '<p class="empty">No unreviewed townhouse listings.</p>';
  } else {
